...
function pickUrbanStoryFormula(params: {
  ...
  const scored = formulaPool.map((formula, index) => {
    ...
    const allowRelationshipDrama = isRelationshipDramaExplicitlyRequested({
      genreLabel: params.genreLabel,
      heroineLabel: params.heroineLabel,
    });
    const relationshipBiasPenalty = getRelationshipDramaBiasPenalty(formulaText, allowRelationshipDrama);
    ...
    return {
      formula,
      score: relationshipBiasPenalty + avoidPenalty + recentFormulaPenalty + seedBias,
    };
  });
...
const RELATIONSHIP_DRAMA_TERMS = [
  "hôn nhân",
  "hôn thê",
  "hủy hôn",
  "ly hôn",
...
function hasAnyBiasTerm(text: string, terms: string[]) {
  const normalized = normalizeBiasText(text);
  return terms.some((term) => normalized.includes(normalizeBiasText(term)));
}

function isRelationshipDramaExplicitlyRequested(params: {
  genreLabel?: string;
  heroineLabel?: string;
}) {
  return hasAnyBiasTerm(
    `${params.genreLabel || ""} ${params.heroineLabel || ""}`,
    RELATIONSHIP_DRAMA_TERMS,
  );
}

function getRelationshipDramaBiasPenalty(text: string, allowRelationshipDrama: boolean) {
  if (allowRelationshipDrama) return 0;
  const normalized = normalizeBiasText(text);
  return RELATIONSHIP_DRAMA_TERMS.reduce((score, term) => {
    return normalized.includes(normalizeBiasText(term)) ? score + 18 : score;
  }, 0);
}

function sanitizeRelationshipDramaLeak(value: string, allowRelationshipDrama: boolean) {
  if (allowRelationshipDrama || !value) return value;

  return value
    .replace(/gia đình chồng/gi, "gia đình đối phương")
    .replace(/nhà chồng/gi, "nhóm trưởng bối")
    .replace(/mẹ chồng/gi, "trưởng bối trong nhà")
    .replace(/người chồng/gi, "người từng đứng về phía cô")
    .replace(/chồng cũ/gi, "người từng hợp tác thân thiết")
    .replace(/chồng/gi, "người thân cũ")
    .replace(/vợ chồng/gi, "quan hệ thân thiết")
    .replace(/hôn nhân hợp đồng/gi, "thỏa thuận hợp tác")
    .replace(/hôn nhân/gi, "thỏa thuận gia đình")
    .replace(/hôn thê/gi, "người được chọn thay thế")
    .replace(/hủy hôn/gi, "hủy thỏa thuận")
    .replace(/ly hôn/gi, "hủy thỏa thuận")
    .replace(/ngoại tình/gi, "bê bối đời tư")
    .replace(/tiểu tam/gi, "người thứ ba trong lời đồn")
    .replace(/vị hôn phu/gi, "người được chỉ định đứng cạnh cô")
    .replace(/cô vợ/gi, "người phụ nữ")
    .replace(/cô dâu/gi, "người phụ nữ trong buổi lễ")
    .replace(/con dâu/gi, "người trong nhà")
    .replace(/chị dâu/gi, "người trong nhà")
    .replace(/em dâu/gi, "người trong nhà");
}
...
function pickDramaLane(seed: string, avoidLibrary?: AvoidLibrary) {
  const candidates = FACTORY_DRAMA_LANES.map((lane, index) => {
    ...
    const relationshipBiasPenalty = getRelationshipDramaBiasPenalty(laneText, false) / 10;
    ...
    return {
      lane,
      score: relationshipBiasPenalty + avoidPenalty + proceduralPenalty + Number(seedBias),
    };
  });
...
function buildSeedCandidate(params: {
  ...
  relationshipConflict = sanitizeRelationshipDramaLeak(relationshipConflict, allowRelationshipDrama);
  setting = sanitizeRelationshipDramaLeak(setting, allowRelationshipDrama);
  evidenceObject = sanitizeRelationshipDramaLeak(evidenceObject, allowRelationshipDrama);
  publicPressure = sanitizeRelationshipDramaLeak(publicPressure, allowRelationshipDrama);
  hiddenTruth = sanitizeRelationshipDramaLeak(hiddenTruth, allowRelationshipDrama);
  villainAttack = sanitizeRelationshipDramaLeak(villainAttack, allowRelationshipDrama);
  heroineCounter = sanitizeRelationshipDramaLeak(heroineCounter, allowRelationshipDrama);
  emotionalStake = sanitizeRelationshipDramaLeak(emotionalStake, allowRelationshipDrama);
  dopamineHook = sanitizeRelationshipDramaLeak(dopamineHook, allowRelationshipDrama);
...
export function buildMockStorySeed(params: {
  ...
  const laneOrder = lanePool.map((lane, index) => {
    ...
    const relationshipBiasPenalty = getRelationshipDramaBiasPenalty(laneText, allowRelationshipDrama) / 10;
    ...
    return {
      lane,
      score: relationshipBiasPenalty + motifBankBonus + genreLaneBonus + avoidPenalty * 0.9 + proceduralPenalty + seedBias,
    };
...
- Không tự kéo truyện về hôn nhân phản bội / hủy hôn / chồng cũ hối hận / ngoại tình / tiểu tam nếu các cụm đó không xuất hiện rõ trong Story Seed, Genre blend hoặc Heroine arc ở trên. Nếu seed là bệnh viện, trường học, công sở, showbiz, logistics, sáng tạo, tài chính, nghệ thuật... thì giữ đúng lõi đó.
