...
function compactTags(input: string, limit = 12) {
  return input
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
...
const MOTIF_TAG_STOPWORDS = new Set([
  ...
  "minh", "thay", "dieu", "noi", "hoi", "kiem", "tra", "tam", "dung",
  "nhom", "mail", "email", "header", "forward", "chuyen", "tiep", "ban",
  "khach", "trao", "than", "trung", "lap", "duoc", "can", "nguon",
]);
...
function countTagOverlap(left: Set<string>, right: Set<string>) {
  let overlap = 0;
  left.forEach((tag) => {
...
function scoreOverlapWithAvoidLibrary(
  ...
  const avoidTexts = [
    ...
    ...(avoidLibrary?.motifFingerprints ?? []).flatMap((item) => [
      item.fingerprint?.premiseFamily,
      item.fingerprint?.openingArena,
      item.fingerprint?.incitingIncident,
      item.fingerprint?.evidenceType,
      item.fingerprint?.evidenceObject,
      item.fingerprint?.villainAttackType,
      item.fingerprint?.heroineCounterType,
      item.fingerprint?.powerStructure,
      item.fingerprint?.publicPressure,
      item.fingerprint?.emotionalWound,
      item.fingerprint?.hiddenTruthType,
      item.fingerprint?.mainArena,
      item.fingerprint?.secondaryArena,
      item.fingerprint?.relationshipCore,
      item.fingerprint?.twistEngine,
      item.fingerprint?.deadlineStyle,
      item.fingerprint?.endingPromise,
      ...(item.fingerprint?.antiRepeatTags ?? []),
    ]),
  ].filter(Boolean);

  if (!avoidTexts.length) return 0;

  const candidateTags = new Set(compactTags(candidateText, 36));
  if (!candidateTags.size) return 0;

  let highestScore = 0;

  for (const avoidText of avoidTexts) {
    const avoidTags = new Set(compactTags(String(avoidText), 36));
    if (!avoidTags.size) continue;

    const overlap = countTagOverlap(candidateTags, avoidTags);
    const score = overlap / Math.max(Math.min(candidateTags.size, avoidTags.size), 6);
    highestScore = Math.max(highestScore, score);

    // Nếu 4+ tag đặc thù trùng nhau, coi là cùng xương sống dù title/vật chứng đổi tên.
    if (overlap >= 4) highestScore = Math.max(highestScore, 0.38 + overlap * 0.035);
    if (overlap >= 7) highestScore = Math.max(highestScore, 0.72);
  }

  return Math.min(highestScore, 1);
}

function scoreCandidateStructuralRepeat(
  candidate: any,
  avoidLibrary?: AvoidLibrary,
) {
  const avoidFingerprints = avoidLibrary?.motifFingerprints ?? [];
  if (!avoidFingerprints.length) return 0;
  ...
  const candidateFields = {
    ...
    heroineCounter: candidate.heroineCounter,
  ...
  for (const item of avoidFingerprints) {
    ...
    let hits = 0;
    for (const value of Object.values(candidateFields)) {
      const normalized = normalizeForCompare(value);
      if (normalized.length >= 14 && text.includes(normalized)) hits += 1;
    }

    const candidateTags = new Set(compactTags(Object.values(candidateFields).join(' | '), 36));
    const avoidTags = new Set(compactTags(text, 36));
    const tagHits = countTagOverlap(candidateTags, avoidTags);

    const score = hits * 0.16 + Math.min(tagHits, 10) * 0.045;
    highest = Math.max(highest, score);
  }

  return Math.min(highest, 1);
}
...
export function buildMockStorySeed(params: {
  ...
  for (const laneItem of laneOrder) {
    for (let attempt = 0; attempt < 8; attempt += 1) {
      ...
      const structuralPenalty = scoreCandidateStructuralRepeat(candidate, params.avoidLibrary);
      ...
      const score = overlapPenalty * 1.15 + structuralPenalty * 2.4 + proceduralPenalty + laneItem.score + exactGenrePenalty;
      ...
      if (overlapPenalty <= 0.16 && structuralPenalty <= 0.18 && proceduralPenalty <= 0.18) break;
    }
  ...
  return {
    ...
    pipeline: {
      ...
      note: "Planner v11 anti-repeat: ưu tiên khác formula + khác arena + khác evidence path; phạt mạnh truyện chỉ đổi tên vật chứng nhưng vẫn lặp chuỗi tố cáo → giấy/email/header/camera → người trung lập tạm dừng.",
    },
  } as FactoryStorySeed;
}
