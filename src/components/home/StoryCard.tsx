...
function getStoryCompletionLabel(story: Story) {
  const raw = String(
  ...
  const hasFullTag = story.tags?.some((tag) => {
    const value = String(tag).trim().toLowerCase()
    return value === "full" || value === "hoàn thành" || value === "hoan thanh"
  })
...
export default function StoryCard({ story }: { story: Story }) {
  ...
  const completionLabel = getStoryCompletionLabel(story)
      ...
      <div className="relative overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900/40 transition duration-200 ease-out group-hover:-translate-y-0.5 group-hover:border-zinc-700 group-hover:bg-zinc-900/55">
        {(() => {
          const raw =
            ...
            (story as any).cover_image ??
            (story as any).cover ??
            (story as any).cover_url ??
            null
          const resolved = resolveCoverUrl(raw)

          if (import.meta.env.DEV) {
            console.debug("[cover debug card]", story.title, "raw:", raw, "resolved:", resolved)
          }
          ...
          return (
            <div className="flex h-[180px] w-full items-center justify-center bg-zinc-900/20 text-zinc-400 sm:h-[240px] lg:h-[260px]">
              No cover
            </div>
          )
        })()}
        ...
        <div className="absolute right-2 top-2 flex gap-2">
          {story.tags?.includes("HOT") ? (
            <div className="rounded bg-amber-500 px-2 py-1 text-[11px] font-bold tracking-wide text-zinc-950">
              HOT
            </div>
          ) : null}
          ...
          <div
            className={
              isFull
                ? "rounded bg-emerald-400/15 px-2 py-1 text-[11px] font-semibold text-emerald-200"
                : "rounded bg-sky-400/15 px-2 py-1 text-[11px] font-semibold text-sky-200"
            }
          >
            {completionLabel}
          </div>
