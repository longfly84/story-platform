...
function getDramaBalanceInstruction() {
  return `
PROCEDURAL FATIGUE LOCK:
- Không được để conflict chính của nhiều chương liên tiếp xoay quanh thủ tục pháp lý/kỹ thuật.
- Các cơ chế sau chỉ được làm yếu tố phụ nếu chương trước đã dùng chúng làm trọng tâm: niêm phong, phong tỏa, bảo toàn chứng cứ, giám định, log hệ thống, quyền truy cập bị khóa, văn bản pháp lý, mã tham chiếu, tòa án, tạm lệnh, lệnh pháp lý.
- Nếu chương trước đã dùng thủ tục pháp lý/kỹ thuật làm cú drama chính, chương hiện tại bắt buộc phải đổi trọng tâm sang ít nhất 1 hướng: phản diện chính đối đầu trực diện, người thân thiết phản bội/lộ sơ hở, truyền thông bẩn/hot search/livestream/họp báo, cảnh emotional thật với con/người thân, nữ chính chủ động gài bẫy ngược, đồng minh đổi phe, bí mật quan hệ bị hé, hoặc cú vả mặt công khai nhỏ.
- Có thể vẫn dùng luật sư, hồ sơ, giám định, log, nhưng chúng không được là cú drama chính của chương nếu chương trước đã dùng rồi.

SCENE VARIETY & DRAMA BALANCE RULE:
- Không được để 3 chương liên tiếp có cùng cấu trúc: nữ chính đến một nơi → bị chặn bằng giấy tờ/pháp lý → luật sư/pháp vụ xuất hiện → nữ chính nói sẽ kiện/giám định.
- Mỗi chương phải có đủ 3 lớp: một tiến triển chứng cứ/tình thế, một va chạm cảm xúc hoặc đối thoại trực diện, và một thay đổi thật về quyền lực.
- Thay đổi quyền lực hợp lệ: ai đó mất lợi thế, đổi phe, bị lộ sơ hở, dư luận đổi chiều, nữ chính giành được thắng lợi nhỏ, hoặc phản diện buộc phải đổi chiến thuật.
- Nếu chương hiện tại chỉ thêm một lớp phong tỏa/niêm phong/log mà không đổi quyền lực, phải tự viết lại chương trước khi trả output.

MAIN VILLAIN PRESENCE RULE:
- Không để luật sư, pháp vụ, PR, trợ lý gánh vai ác quá lâu.
- Trong mỗi 2 chương liên tiếp, phản diện chính phải hiện diện ít nhất 1 lần qua đối thoại trực diện, cuộc gọi đe dọa, họp hội đồng, livestream/hot search, tin nhắn gây áp lực cá nhân, hoặc một đòn phản công có chữ ký cá nhân rõ ràng.
- Phản diện chính phải có cá tính, mục tiêu và cách sỉ nhục riêng; không chỉ nói chung chung kiểu “theo quy trình”, “bảo toàn chứng cứ”, “cô nên cân nhắc”.
- Nếu chương đã có phản diện phụ/luật sư xuất hiện, phải làm rõ họ chỉ là cánh tay của phản diện chính, không thay phản diện chính làm trung tâm xung đột quá lâu.

CHILD EMOTIONAL STAKE RULE:
- Nếu truyện có con nhỏ/người thân yếu thế bị kéo vào conflict, mỗi 1–2 chương phải có một cảnh ngắn cho thấy tác động thật.
- Không chỉ kể “con bị ảnh hưởng”. Phải có scene cụ thể: con hỏi mẹ một câu đau, con bị bạn bè xa lánh, con bị giáo viên gọi ra khỏi lớp, con sợ mình làm sai, mẹ phải trấn an con nhưng không được yếu đuối, hoặc nữ chính biến cảm xúc đó thành quyết tâm phản công.
- Cảnh này không được dài dòng, nhưng phải khiến người đọc thấy nữ chính có lý do để phản công mạnh hơn.

DOPAMINE BEAT RULE:
- Mỗi chương phải có ít nhất một nhịp khiến độc giả muốn đọc tiếp: một câu thoại sắc, một cú nhục mặt nhỏ, một người đổi phe, một bằng chứng lật nghĩa, một hành động chủ động của nữ chính, hoặc một phản ứng công khai khiến phe phản diện mất bình tĩnh.
- Không được để chương chỉ toàn giải thích thủ tục, đọc giấy tờ, ghi nhận công văn, rồi kết bằng quyết tâm nội tâm.
`.trim()
}
...
=== DRAMA BALANCE CHECK ===
- Conflict chính của chương:
- Chương này có lặp niêm phong/phong tỏa/giám định/log làm conflict chính không:
- Nếu có yếu tố pháp lý/kỹ thuật, nó là conflict chính hay phụ:
- Va chạm cảm xúc hoặc đối thoại trực diện chính:
- Phản diện chính có hiện diện trực tiếp hoặc có đòn có chữ ký cá nhân không:
- Tình thế/quyền lực đã thay đổi thật ở điểm nào:
- Cú dopamine/vả mặt/twist nhỏ của chương:
- Chương sau nên tránh lặp cơ chế nào:
...
function buildChapterPrompt(payload: NormalizedGeneratePayload) {
  ...
  const dramaBalanceInstruction = getDramaBalanceInstruction()
...
8. Conflict chính có lại là niêm phong/phong tỏa/giám định/log sau khi chương trước vừa dùng không? Nếu có, đổi trọng tâm sang đối đầu, truyền thông, phản bội, cảm xúc hoặc gài bẫy.
9. Phản diện chính có hiện diện hoặc để lại đòn có chữ ký cá nhân chưa? Nếu chưa, thêm áp lực rõ từ phản diện chính.
10. Có cảnh emotional stake nếu truyện có con/người thân yếu thế không? Nếu chưa, thêm một cảnh ngắn nhưng đau.
11. Có dopamine beat/cú vả mặt/twist nhỏ không? Nếu chưa, thêm một nhịp cụ thể.
12. Có câu nào giống phân tích kỹ thuật trong BẢN ĐỌC không? Nếu có, phải biến thành cảnh.
13. Có đoạn nào lan man, chỉ giải thích mà không đẩy truyện không? Nếu có, cắt hoặc viết lại.
14. Kết chương có đủ hook để đọc tiếp không? Nếu chưa, viết lại đoạn cuối.
15. BẢN ĐỌC có đọc như truyện thật không? Nếu còn như outline/tóm tắt, viết lại thành cảnh có hành động và đối thoại.
16. Phần kỹ thuật có điền đầy đủ scene function, chapter mission, địa điểm chính, hành động mới, state change, drama balance check và chương sau nên đi đâu chưa?
